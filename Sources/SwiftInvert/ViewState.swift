...
///
/// Use `@ViewState` everywhere you would have written `@State`; in a custom
/// `init`, assign `_field = ViewState(wrappedValue: ...)` where you would
/// have written `State(initialValue: ...)`. The Makefile rejects `@State`
/// so this can't silently regress on a machine that happens to have Xcode.
/// Reference implementation: ~/Documents/code/claude-status.
@propertyWrapper
